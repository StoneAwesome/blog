...
export const InstagramBody: React.FC<Props> = ({ title, date, children, tags, material, post }) => {
  ...
  return (
    <BasicContainer>
      <div className={``}>
        <div className={"bg-_bsLight border-b-2 border-l-2 border-r-2 p-2 "}>
          <header>
        ...
        {children && <div className={`border-b-2 md:border-r-2 md:border-l-2`}>{children}</div>}
        {post && (
          <div className={`flex flex-col md:grid md:grid-cols-12 md:divide-x`}>
            <div className={`md:col-span-5`}>
              <InstagramMediaViewer post={post} />
            </div>
            {post.caption && (
              <div className={`md:col-span-7`}>
                <div className={`p-3 prose prose-a:text-_bsPrimary`}>
...
const SimpleInstagramEmbed: React.FC<{ url: string }> = (props) => {
  return (
    <>
      <Script async src="//www.instagram.com/embed.js" key={"INSTAGRAM_EMBED"} />
      <div className={"d-flex flex-column align-items-center"}>
        <blockquote
          className="instagram-media"
          data-instgrm-captioned
          data-instgrm-permalink={`${props.url}?utm_source=ig_embed&utm_campaign=loading`}
        ...
        >
          <div style={{ padding: "16px" }}>
            <a
              href={`${props.url}?utm_source=ig_embed&utm_campaign=loading`}
              style={{
                background: "#FFFFFF",
                lineHeight: 0,
                padding: "0 0",
